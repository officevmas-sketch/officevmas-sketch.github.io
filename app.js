...
async function enforceSingleLogin() {
  const deviceId = getDeviceId();

  const sessionRef = db.ref(
    `companies/${session.company}/activeSessions/${session.user}`
  );
...
async function login() {
  ...
  const allowed = await enforceSingleLogin();
...
function renderEmployees() {
  ...
  if (!list.innerHTML) {
    list.innerHTML = '<p class="muted">No active employees found.</p>';
  }
}
...
function listenCalls() {
  ...
  activeCallRef.on("child_added", snap => {
    const call = snap.val();
...
async function addUser() {
  if (session.role !== "companyAdmin") {
    alert("Only company admin can add users.");
    return;
  }
